    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white flex flex-col">
      <header className="w-full py-6 px-4 md:px-8 flex justify-between items-center bg-black/30 backdrop-blur-sm">
        ...
        <nav className="space-x-6">
          ...
          <Link href="/discover" className="text-white hover:text-pink-400 transition">
            Discover
          </Link>
      ...
      <section className="text-center mt-16 px-4 max-w-5xl mx-auto">
        ...
        <p className="text-xl text-gray-300 mb-10 max-w-3xl mx-auto">
          Find and book tickets for concerts, festivals, sports, and more with our AI-powered event discovery platform.
        </p>
        <div className="flex flex-wrap justify-center gap-4">
          <Link href="/events" className="bg-pink-600 hover:bg-pink-700 text-white px-8 py-3 rounded-xl text-lg font-medium transition">
            Browse Events
          </Link>
          <Link href="/ai-assistant" className="bg-transparent border-2 border-pink-500 hover:bg-pink-500/20 text-white px-8 py-3 rounded-xl text-lg font-medium transition">
            Ask AI
          </Link>
        </div>
      </section>
      ...
      <section className="mt-24 px-4 max-w-6xl mx-auto w-full">
        <h3 className="text-2xl font-semibold mb-8 text-center">Featured Events</h3>
            ...
            {featuredEvents.map((event) => (
              <Link href={`/events/${event.id}`} key={event.id} className="group">
                <div className="bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-pink-500/20 transition">
                  ...
                  <div className="p-4">
                    <p className="text-xs font-medium text-pink-400">{event.type}</p>
                    <h4 className="text-lg font-semibold mt-1 group-hover:text-pink-400 transition">{event.title}</h4>
        ...
        <div className="text-center mt-12">
          <Link href="/events" className="text-pink-400 hover:text-pink-300 inline-flex items-center">
            View all events
